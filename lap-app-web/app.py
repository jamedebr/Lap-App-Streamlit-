...
RADIUS_DEG = 0.00015       # ~11 m — increase if GPS drift causes missed checkpoints
    ...
    with col1:
        if st.button("Sign In", use_container_width=True):
            ...
            result = (
                ...
                .select("*")
                .eq("email", email)
                .eq("password", hash_password(password)) #.eq("password", hash_password(password))
                .execute()
            )
            if result.data:
    ...
    with col1:
        if st.button("Submit", use_container_width=True):
            ...
            client.table("accounts").insert(
                {"email": email, "password": hash_password(password), "laps": 0} #{"email": email, "password": hash_password(password), "laps": 0}
            ).execute()
            st.session_state.user_email = email
            st.session_state.laps_offset = 0
            st.session_state.page = "home"
